/* STEP 1: Define ADA File Path */
%let file_path = /home/u64447590/ADA/Cardano_ready.csv;
...
/* STEP 3: Plot ADA vs DXY */
title "Cardano (ADA) Price and Dollar Index (DXY) Comparison";
proc sgplot data=work.ada_data;
    series x=Date_sas y=Price_num / lineattrs=(thickness=2 color=blue) 
           name="ada_line" legendlabel="ADA Price";
    ...
    yaxis label="ADA Price (Log Scale)" type=log grid;
    y2axis label="Dollar Index (DXY) Value" grid;
    keylegend "ada_line" "dxy_line" / position=topright;
run;
title;