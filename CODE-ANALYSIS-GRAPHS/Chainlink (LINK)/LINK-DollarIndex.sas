/* STEP 1: Define LINK File Path */
%let file_path = /home/u64447590/LINK/Chainlink_ready.csv;
...
data work.link_data;
    ...
    /* Convert date and handle comma-decimal conversion */
...
/* STEP 3: Plot LINK vs DXY */
title "Chainlink (LINK) Price and Dollar Index (DXY) Comparison";
proc sgplot data=work.link_data;
    /* Primary Y-Axis: LINK Price (Logarithmic) */
    series x=Date_sas y=Price_num / lineattrs=(thickness=2 color=blue) 
           name="link_line" legendlabel="LINK Price";
    ...
    yaxis label="LINK Price (Log Scale)" type=log grid;
    y2axis label="Dollar Index (DXY) Value" grid;
    keylegend "link_line" "dxy_line" / position=topright;
run;
title;