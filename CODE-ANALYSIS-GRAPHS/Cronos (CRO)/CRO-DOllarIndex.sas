/* STEP 1: Define CRO File Path */
%let file_path = /home/u64447590/CRO/Cronos_ready.csv;
...
/* STEP 3: Plot CRO vs DXY */
title "Cronos (CRO) Price and Dollar Index (DXY) Comparison";
proc sgplot data=work.cro_data;
    /* Primary Y-Axis: CRO Price (Logarithmic) */
    series x=Date_sas y=Price_num / lineattrs=(thickness=2 color=blue) 
           name="cro_line" legendlabel="CRO Price";
    ...
    yaxis label="CRO Price (Log Scale)" type=log grid;
    y2axis label="Dollar Index (DXY) Value" grid;
    keylegend "cro_line" "dxy_line" / position=topright;
run;
title;