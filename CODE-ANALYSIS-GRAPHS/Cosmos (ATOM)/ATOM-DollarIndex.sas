/* STEP 1: Define ATOM File Path */
%let file_path = /home/u64447590/ATOM/Cosmos_ready.csv;
...
/* STEP 3: Plot ATOM vs DXY */
title "Cosmos (ATOM) Price and Dollar Index (DXY) Comparison";
proc sgplot data=work.atom_data;
    /* Primary Y-Axis: ATOM Price (Logarithmic) */
    series x=Date_sas y=Price_num / lineattrs=(thickness=2 color=blue) 
           name="atom_line" legendlabel="ATOM Price";
    ...
    yaxis label="ATOM Price (Log Scale)" type=log grid;
    y2axis label="Dollar Index (DXY) Value" grid;
    keylegend "atom_line" "dxy_line" / position=topright;
run;
title;