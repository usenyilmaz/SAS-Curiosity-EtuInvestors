/* STEP 1: Define BNB File Path */
%let file_path = /home/u64447590/BNB/BinanceCoin_ready.csv;
...
/* STEP 3: Plot BNB vs DXY */
title "Binance Coin (BNB) Price and Dollar Index (DXY) Comparison";
proc sgplot data=work.bnb_data;
    series x=Date_sas y=Price_num / lineattrs=(thickness=2 color=blue) 
           name="bnb_line" legendlabel="BNB Price";
    ...
    yaxis label="BNB Price (Log Scale)" type=log grid;
    y2axis label="Dollar Index (DXY) Value" grid;
    keylegend "bnb_line" "dxy_line" / position=topright;
run;
title;