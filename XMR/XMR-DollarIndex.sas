/* STEP 1: Define XMR File Path */
%let file_path = /home/u64447590/XMR/Monero_ready.csv;
...
/* STEP 3: Plot XMR vs DXY */
title "Monero (XMR) Price and Dollar Index (DXY) Comparison";
proc sgplot data=work.xmr_data;
    /* Primary Y-Axis: XMR Price (Logarithmic) */
    series x=Date_sas y=Price_num / lineattrs=(thickness=2 color=blue) 
           name="xmr_line" legendlabel="XMR Price";
    ...
    yaxis label="XMR Price (Log Scale)" type=log grid;
    y2axis label="Dollar Index (DXY) Value" grid;
    keylegend "xmr_line" "dxy_line" / position=topright;
run;
title;