/* Adapted from CODE-ANALYSIS-GRAPHS/CryptoCoins/Bitcoin (BTC)/BTC-M2SL.sas
   Only edit: hardcoded /home/u64447590/BTC/btc_ready.csv path rewritten to ./input/btc_ready.csv. */

/* STEP 1: Data Processing (Reading M2SLCHANGE) */
data work.btc_m2;
    infile "./input/btc_ready.csv" dlm=';' firstobs=2 dsd;
    ...
    format Date_sas date9. Price_num M2_num 12.2;
    keep Date_sas Price_num M2_num;
run;

/* STEP 2: Plot BTC vs M2 */
title "Bitcoin (BTC) Price and M2 Money Supply Comparison";
proc sgplot data=work.btc_m2;
    ...
    keylegend "btc_line" "m2_line" / position=topright;
run;
title;

proc print data=work.btc_m2(obs=12); run;
